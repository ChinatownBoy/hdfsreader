    ...
    private void addSourceFileByType(String filePath) {
        ...
        if (isMatchedFileType) {
            ...
            String message = String.format("文件[%s]的类型与用户配置的fileType类型不一致，" +
                    "请确认您配置的目录下面所有文件的类型均为[%s]", filePath, this.specifiedFileType);
            LOG.error(message);
    ...
    public void orcFileStartRead(String sourceOrcFilePath, Configuration readerSliceConfig,
        ...
        if (columnIndexMax >= 0) {
            ...
            try {
                ...
                // If the network disconnected, will retry 45 times, each time the retry
                // interval for 20 seconds
                // Each file as a split
                // TODO multy threads
                // OrcInputFormat getSplits params numSplits not used, splits size = block
                // numbers
                ...
                for (InputSplit split : splits) {
                    {
                        ...
                        List<ColumnEntry> fields = UnstructuredStorageReaderUtil
                        .getListColumnEntry(readerSliceConfig, com.alibaba.datax.plugin.unstructuredstorage.reader.Key.COLUMN);
                        ...
                        for (int i = 0; i <= columnIndexMax; i++) {
                        Object field = inspector.getStructFieldData(value, inspector.getStructFieldRef(fields.get(i).getValue()));
